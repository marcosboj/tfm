...
def crear_carpetas_project(project_root: Path):
    clustering_dir = project_root / "resultados" / "clustering_resumen"
    pca_dir = project_root / "resultados" / "pca_resumen"
...
def main(resumen_dir: Path, project_root: Path):
    ...
    df_met.to_csv(project_root / 'resultados' / 'cluster_metrics_resumen.csv', index=False)
    print('Clustering de resúmenes completado. Métricas guardadas en resultados/cluster_metrics_resumen.csv')

    # Guardar feature importances
    df_imp = pd.DataFrame(feature_imps)
    df_imp.to_csv(project_root / 'resultados' / 'feature_importances_resumen.csv', index=False)
    ...
    df_all.to_csv(project_root / 'resultados' / 'all_labels_unified_resumen.csv', index=False)
    print('All labels unified saved to resultados/all_labels_unified_resumen.csv')
